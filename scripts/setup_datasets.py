...
def extract_stemmable_words_from_wiki(
    ...
    # Sort so the returned order is deterministic across runs: set iteration
    # order is not stable, and generate_stemmable_dataset() indexes into this
    # list with a seeded RNG, so a stable order is required for reproducibility.
    return sorted(stemmable)
